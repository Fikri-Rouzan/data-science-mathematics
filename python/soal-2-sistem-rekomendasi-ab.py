...
Original file is located at
    https://colab.research.google.com/drive/1h2p2fLfgxNo6afv6nQn2nCSSPkbqjb_Q
...
def generate_df_ab_test(n_days=20):
  ...
  all_user_ids = create_unique_user_ids(total_users)
...
def degrees_of_freedom(origin_metrics, varian_metrics):
  ...
  numerator = np.square(v1 + v2)

  denominator = (np.square(v1) / (n1 - 1)) + (np.square(v2) / (n2 - 1))
...
def t_statistics_form(origin_metrics, varian_metrics):
    ...
    denominator = np.sqrt((np.square(s1) / n1) + (np.square(s2) / n2))
